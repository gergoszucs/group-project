...
MyGLYTrajectoryView::MyGLYTrajectoryView(QWidget *parent)
...
MyGLYTrajectoryView::~MyGLYTrajectoryView(void)
{
}

void MyGLYTrajectoryView::keyPressEvent( QKeyEvent * event )
{
	...
	{
		glYPanCentreY = glYPanCentreY + factor;
	}
	...
	{
		glYPanCentreX = glYPanCentreX - factor;
	}
...
void MyGLYTrajectoryView::mouseMoveEvent(QMouseEvent *event)
{
	...
	if (event->modifiers() & Qt::ShiftModifier)
	{
		...
		if (MY_WIDGET_CONTROL == Y) // Only zoom if we have control.
		{
			...
			// Update zoom.
			glYViewHalfExtent = glYViewHalfExtent + dy;

			lastPos = event->pos();

		} // End of if (MY_WIDGET_CONTROL == Y) 
	}
...
void MyGLYTrajectoryView::dragFocusPoints(float posX, float posY, float old_posX, float old_posY)
{
	for (int k=0; k<project->get_MySurfaces()->size(); k++)
	{

		int n = project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyFocusTrajectoryCurveSegmentIndices()->size();
		...
		int noOfSegments = project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->size();

		// Loop over focus end points.
		for (int t=0; t<n; t++)
		{

			int index = project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyFocusTrajectoryCurveSegmentIndices()->at(t);
			ITPointTrajectory *p1 = project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->at(index)->get_P1_p();
			...
			{
				ITPointTrajectory *p0 = project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->at(index+1)->get_P0_p();
		...
		// Update curve.
		project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->computeMySegmentEndTangentVectors();

	} // End of k loop

} // End of dragFocusPoints.

void MyGLYTrajectoryView::mouseReleaseEvent(QMouseEvent *releaseEvent)
{
	...
	if (MY_EDIT_MODE == DRAG_TRAJECTORY_POINT)
	{
		...
		for (int k=0; k<project->get_MySurfaces()->size(); k++)
		{
			...
			for (int t=0; t<n; t++)
			{
				project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyFocusTrajectoryCurveSegmentIndices()->pop_back();
...
void MyGLYTrajectoryView::mousePressEvent(QMouseEvent *event)
{
...
void MyGLYTrajectoryView::AssignFocusPoint(QMouseEvent *event)
{
	...
	if ( (k>-1) && (i>-1) )
	{
		...
		if (MY_EDIT_MODE == DRAG_TRAJECTORY_POINT)
		{
			...
			project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyFocusTrajectoryCurveSegmentIndices()->push_back(i);
		}
...
void  MyGLYTrajectoryView::findTrajectoryPointIndicesNearMouse(double posX, double posY, double posZ, int *targetK, int *targetI)
{
	float threshold = 1.0;
	...
	*targetK = -1;
	...
	for (int k=0; k<project->get_MySurfaces()->size(); k++)
	{
		for (int i=0; i<project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->size(); i++)
		{
			ITTrajectoryCurveSegment* currentSegment = (ITTrajectoryCurveSegment*)(project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->at(i));
...
void MyGLYTrajectoryView::setViewOrtho(int width, int height)
{
	...
	glOrtho(glYPanCentreX - glYViewHalfExtent, 
			glYPanCentreX + glYViewHalfExtent,
			glYPanCentreY - glYViewHalfExtent / aspect, 
			glYPanCentreY + glYViewHalfExtent / aspect, 
			-50000.0, 
			50000.0);
...
void MyGLYTrajectoryView::resizeGL(int width, int height)
{
	myWidth = width;
	myHeight = height;
...
void MyGLYTrajectoryView::paintGL()
{
...
void MyGLYTrajectoryView::initializeGL()
{
...
void MyGLYTrajectoryView::findOrdinateRange(float *minOrdinate, float *maxOrdinate)
{
	*maxOrdinate = -10000.0;
	*minOrdinate = 10000.0;
	...
	for (int k=0; k<project->get_MySurfaces()->size(); k++)
	{
		...
		project->printDebug(__FILE__, __LINE__, __FUNCTION__, 12, "No of curve segments: %i", project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->size());
...
void MyGLYTrajectoryView::drawMyCurveHandles(int curveIndex)
{
...
void MyGLYTrajectoryView::drawMyCurve()
{
	...
	for (int k=0; k<project->get_MySurfaces()->size(); k++)
	{
		...
		ITTrajectoryCurveSegment *currentSegment = (ITTrajectoryCurveSegment *)(project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->at(0));
		...
		if (drawAnnotations)
		{
			ITTrajectoryCurveSegment *currentSegment = (ITTrajectoryCurveSegment *)(project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->back());
			float plotX = currentSegment->get_EndKeyFrame();
			float plotY = currentSegment->get_P1_p()->get_X();
	...
	// Now plot the dots.
	// Loop though all the surfaces.
	float radius = glYViewHalfExtent/100.0;
...
void MyGLYTrajectoryView::drawMyFocusSegments()
{

	for (int k=0; k<project->get_MySurfaces()->size(); k++)
	{

		for (int t=0; t<project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyFocusTrajectoryCurveSegmentIndices()->size(); t++)
		{
			int index = project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyFocusTrajectoryCurveSegmentIndices()->at(t);
			ITTrajectoryCurveSegment *s = project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->at(index);


			glTranslatef(s->get_EndKeyFrame(), s->get_P1_p()->get_X(), 0.0);

			float radius = glYViewHalfExtent/50.0;
...
void MyGLYTrajectoryView::drawMyAnnotations()
{
	...
	renderText(0.0, maxOrdinate+5.0, 0.0, "Y-translation");
	...
	for (int i=0; i<project->get_MySurfaces()->at(0)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->size(); i++)
	{
		ITTrajectoryCurveSegment* currentSegment = project->get_MySurfaces()->at(0)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->at(i);
...
void MyGLYTrajectoryView::drawMyAxes()
{
	...
	for (int i=0; i<project->get_MySurfaces()->at(0)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->size(); i++)
	{
		...
		ITTrajectoryCurveSegment* currentSegment = (ITTrajectoryCurveSegment*)(project->get_MySurfaces()->at(0)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->at(i));
		float plotX = currentSegment->get_EndKeyFrame();
...
void MyGLYTrajectoryView::drawMyInterpolatedCurve(int curveIndex)
{
...
void MyGLYTrajectoryView::drawSphere(double r, int lats, int longs, float R, float GG, float B) 
{
...
void MyGLYTrajectoryView::resetMyView()
{

	glYViewHalfExtent = 50.0;
	glYPanCentreX = 40.0;
	glYPanCentreY = 0.0;
...
// Accessors.
float MyGLYTrajectoryView::get_EditValue(){ return _EditValue; }
void MyGLYTrajectoryView::set_EditValue(float a){ _EditValue = a; }

float MyGLYTrajectoryView::get_MaxY(){ return _MaxY; }
void MyGLYTrajectoryView::set_MaxY(float y){ _MaxY = y; }
