...
MyGLYTrajectoryView::MyGLYTrajectoryView(QWidget *parent) : QGLWidget(QGLFormat(QGL::SampleBuffers), parent)
{
	...
	*minOrdinate = 10000.0;
	...
	for (int k = 0; k < project->get_MySurfaces()->size(); k++)
	{
		project->printDebug(__FILE__, __LINE__, __FUNCTION__, 12, "No of curve segments: %i", 
			project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(1)->get_MyTrajectoryCurveSegments()->size());
