...
void MyGLXTrajectoryView::mouseMoveEvent(QMouseEvent *event)
{
	...
	{
		if (MY_WIDGET_CONTROL == X) // Only zoom if we have control.
		{
...
void MyGLXTrajectoryView::drawMyCurve()
{
	...
	for (int k=0; k<project->get_MySurfaces()->size(); k++)
	{
		...
		ITTrajectoryCurveSegment *currentSegment = (ITTrajectoryCurveSegment *)(project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(0)->get_MyTrajectoryCurveSegments()->at(0));
		...
		if (drawAnnotations)
		{
			ITTrajectoryCurveSegment *currentSegment = (ITTrajectoryCurveSegment *)(project->get_MySurfaces()->at(k)->get_MyTrajectoryCurves()->at(0)->get_MyTrajectoryCurveSegments()->back());
			float plotX = currentSegment->get_EndKeyFrame();
			float plotY = currentSegment->get_P1_p()->get_X();
