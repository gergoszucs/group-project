	...
	if ( (k>-1) && (i>-1) && (j>-1) )
	{
		...
		else if (MY_EDIT_MODE == MERGE_SURFACES_BY_ROW)
		{
			...
			else if (get_PrimedForSecondClick())
			{
				...
				} // End of if numbers of columns are equal.
				else
				{
					QMessageBox::StandardButton reply;
		...
		else if (MY_EDIT_MODE == MERGE_SURFACES_BY_ROW_REVERSE)
		{
			...
			} // End of if get_PrimedForSecondClick().

		} // End of MERGE_SURFACES_BY_ROW_REVERSE.
		else if (MY_EDIT_MODE == MEASURE_DISTANCE)
		{
...
} // End of mergeSurfacesByRowReverse.
...
void MyXYView::mouseMoveEvent(QMouseEvent *event)
{
	...
	if (event->modifiers() & Qt::ShiftModifier)
	{
		...
		// Update zoom.
		glXYViewHalfExtent = glXYViewHalfExtent + dy;
...
} // End of flipSurfaceHorizontally.
...
} // End of perspectiveFocusPointsOld.
...
void MyXYView::dragFocusPoints(float posX, float posY, float old_posX, float old_posY)
{
	for (int k=0; k<project->get_MySurfaces()->size(); k++)
	{
		...
		} // End of of focus point vector contains points.
...
//				p->set_X( p->get_X() + (p->get_X() * p->get_Y())*dy );
		...
		} // End of if there are focus points.

	} // End of k loop.

} // End of shearFocusPoints
...
void MyXYView::resizeFocusPoints(float y, float yold)
{
	...
	for (int k=0; k<project->get_MySurfaces()->size(); k++)
	{
		...
		} // End of if focus vector contains points.
	} // End of k loop.
} // End of resizeFocusPoints.
